"""Request the REST reads that no client drives, against the fixture backend.

Five GETs were dark after a full browser suite, a full read conformance run and both write walks. They
were not waiting on a harness — the GUI conformance register is empty and every port method is driven —
so nothing was going to reach them by accident. They divide in three:
