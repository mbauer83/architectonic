...
## An aggregate is a rollup, not a row with its own obligation
...
**It holds at every level of the chain.** An aggregate *requirement* whose constituents are realized is
realized; reporting it as unrealized is the same defect one level down. ArchiMate's derivation rules
cannot supply this one — an aggregation and a realization that both point *at* the part compose along
no path — so the rollup is what carries it.
...
There is no endpoint type: it is the row's own type, whatever the row is. That is what "peers" means,
and encoding it structurally removes both the repetition of declaring it per level and any way to
declare it wrongly.

What that changes, and nothing else:

- A row with constituents **drops the obligation that asks about itself** — the goal with no outcome of
  its own, the outcome with no requirement of its own, the requirement asking to be realized — and
  carries its constituents' obligations instead. Under the same universal quantification, the row then
  passes exactly when every constituent is covered: no new quantifier, and no second way to express one
  chain.
...
- **An element that aggregates nothing is unchanged**: a goal with no outcome is still
  `missing-outcome`, a requirement is still its own obligation. This is why the rollup is not a branch
  — as a branch it would be universally quantified and make aggregation mandatory everywhere, failing
  every leaf.
