...
Subcommands:
  stats     [--repo PATH]
      Print counts by domain and connection type.

  entities  [--repo PATH] [--domain DOMAIN] [--type TYPE] [--status STATUS]
      List entities matching the given filters.

  connections [--repo PATH] [--type TYPE] [--source ID] [--target ID]
      List connections matching the given filters.

  diagrams  [--repo PATH] [--type TYPE] [--status STATUS]
      List diagrams matching the given filters.

  get       [--repo PATH] ID
      Print full details for an entity, connection, or diagram.

  graph     [--repo PATH] [--hops N] [--type TYPE] ID
      Show connections and N-hop neighbors for an entity.

  search    [--repo PATH] [--limit N] [--domain DOMAIN] [--entities-only]
...
def fmt_diagram(rec: DiagramRecord) -> str:
    return str(rec)
...
def _cmd_stats(registry: Any, args: list[str]) -> int:
    ...
    entities_by_domain = stats.get("entities_by_domain", {})
    ...
    for domain, count in sorted(_safe_items(entities_by_domain)):
        print(f"    {domain:20s} {count}")
    print("  Connections by type:")
    for conn_type, count in sorted(_safe_items(connections_by_type)):
        print(f"    {conn_type:20s} {count}")
    return 0


def _cmd_entities(registry: Any, args: list[str]) -> int:
    domain_val, args = flag(args, "--domain")
...
def _cmd_graph(registry: Any, args: list[str]) -> int:
    ...
    if not args:
        print("Usage: graph [--hops N] [--type TYPE] ID", file=sys.stderr)
        return 1
    entity_id = args[0]
    ...
    _print_direct_connections(registry, entity_id=entity_id, conn_type=type_val)
    if max_hops > 1:
        _print_neighbors(registry, entity_id=entity_id, max_hops=max_hops, conn_type=type_val)
    return 0


def _print_direct_connections(registry: Any, *, entity_id: str, conn_type: str | None) -> None:
    conns = registry.find_connections_for(entity_id, conn_type=conn_type)
    if not conns:
        print("  (none)")
        return
    for conn in conns:
...
def _cmd_search(registry: Any, args: list[str]) -> int:
    ...
    result = registry.search(
        query,
        limit=limit,
        domains=[domain_val] if domain_val else None,
