...
def register(mcp: FastMCP) -> None:
    ...
    register_mutation_tool(
        ...
        description=(
            ...
            "Returns one object for the batch: operation_id, dry_run, committed, item_count, "
            "failed_count, counts (items per op), refs (every '_ref' alias against the artifact_id it "
            "was allocated), and items.\n"
            "return_mode controls how much 'items' carries — 'summary' (default) lists only items "
            "with a warning or an error, so a clean fifty-item batch answers with its refs and counts; "
            "'full' lists every item in input order (op, artifact_id, path, wrote, verification, "
            "warnings?, error?); 'ids' lists none, leaving refs and counts alone. Read the aliases off "
            "'refs' rather than correlating by position — the auto-sort makes input order a weak "
            "contract. Optional idempotency_key reuses a prior completed result for the same logical "
            "batch, re-shaped to the return_mode this call asked for. No file content in results. "
