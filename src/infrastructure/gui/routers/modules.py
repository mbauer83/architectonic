...
from src.infrastructure.gui.contracts.catalog import LoadedModuleListResponse
...
@router.get("/api/modules", tags=[TAG_TAXONOMY], summary="Loaded ontology / diagram-type modules",
    response_model=LoadedModuleListResponse)
def list_modules(catalogs: RuntimeCatalogs = Depends(runtime_catalogs_dependency)) -> dict[str, object]:
