...
def teardown_steps(sync_mgr: object) -> list[TeardownStep]:
    ...
    3. **Release the assurance store** — locks it, which checkpoints its write-ahead log.
    4. **Stop git-sync** — last: it is the only step whose work is external, and nothing above needs
       it to have stopped.
