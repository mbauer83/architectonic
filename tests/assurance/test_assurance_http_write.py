...
class _FakeStore:
    ...
    def list_participating_analyses(self, node_id: str) -> list[str]:
        ...
        surfacing as an unhandled 500 rather than a failure that says what is wrong.
        """
    ...
    def remove_analysis_member(self, analysis_id: str, node_id: str) -> None:
        self._members[node_id] = [
            a for a in self._members.get(node_id, []) if a != analysis_id
        ]
...
_CTX_PATH_READ = "src.infrastructure.rest.routers.assurance._read.get_assurance_context"
...
_CTX_PATH_ARCHIVE = (
    "src.infrastructure.rest.routers.assurance._archive_routes.get_assurance_context"
)
...
def test_model_this_default_creates_and_binds() -> None:
    ...
    with patch(
        "src.infrastructure.rest.routers.assurance._write.GuiArchitectureEntityCreator",
        _StubCreator,
    ):
        resp = client.post("/api/assurance/model-this", json={
            "assurance_node_id": nid,
