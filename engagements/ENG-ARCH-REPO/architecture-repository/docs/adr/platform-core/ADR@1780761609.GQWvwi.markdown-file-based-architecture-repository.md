---
artifact-id: ADR@1780761609.GQWvwi.markdown-file-based-architecture-repository
artifact-type: document
doc-type: adr
title: Markdown File-Based Architecture Repository
status: accepted
version: 0.1.0
...
following the conventions defined in the repository's design decisions.

Entity files use the naming convention `TYPE@epoch.random.friendly-name.md`; the identity scheme
and its directory-based grouping are recorded in
[@DOC:ADR@1783406715.vX4p7z.artifact-identity-typed-epoch-random-identifiers-in-filenames-grouping-by-directory#decision](ADR@1783406715.vX4p7z.artifact-identity-typed-epoch-random-identifiers-in-filenames-grouping-by-directory.md).
...
- Concurrent writes from multiple agents require coordination to avoid merge conflicts in
  `.outgoing.md` files — mitigated by the single serialized write pipeline of
  [@DOC:ADR@1783406851.pGCuZn.one-unified-backend-authority-every-write-through-the-same-verified-pipeline#decision](ADR@1783406851.pGCuZn.one-unified-backend-authority-every-write-through-the-same-verified-pipeline.md).
